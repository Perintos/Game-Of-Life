...
public class PanelGraphics extends JPanel implements ActionListener, MouseListener, MouseWheelListener, MouseMotionListener {
	...
	public PanelGraphics() {				
		...
		iterationManager.addTimer(new Timer(rl.readTimer(), this));		//ajout d'un timer au regle du jeu, le timer est connecte à cette class et à sa methodez actionPerformed
...
//		System.out.println("-----------------------------------------------------------------------");
			...
			{
				this.grille.setPosX(this.grille.getPosX() - (this.mousePressClickX - e.getX()));	// On soustrait à x la distance entre le x initiale (debut deplacement) et le x final (fin de deplacement). 
				this.grille.setPosY(this.grille.getPosY() - (this.mousePressClickY - e.getY()));	// On soustrait à y la distance entre le y initiale (debut deplacement) et le y final (fin de deplacement). 
				repaint();
			}
		}
	}
